...
export const [DMDefaultProps, DMDialogues] = createDialogue({
	entity: 'DM',
})(({ queueDialogue, setOptions, runDialogue, clearPortrait }) => ({
	intro: {
		text: [
	...
	playerUsesScroll: {
		...
		beforeNext: async () => await clearPortrait('MP1'),
	},
	gruzAppears: [
		{
			text: [
			...
			// onEnd: () => queueDialogue('DM', 'gruzAppears_2'),
		},
		{
			text: [
	...
	gruzGrabsSheep: {
		...
		onEnd: () => queueDialogue('GZ', 'gruzLeaves'),
	},
	playerAbandonsSheep: {
	...
	playerAbandonsSheep_recheck: {
		...
		onEnd: () => queueDialogue('MP2', 'iSeeTracks'),
	},
	chaseGuz: {
		text: [
		...
		onEnd: () => queueDialogue('DM', 'onForestPath'),
	},
	playerLeavesGuzAlone: {
		text: [
